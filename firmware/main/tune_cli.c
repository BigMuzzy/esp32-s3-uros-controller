 ...
 *   - each tick (~5 ms): refreshes the can_task tune override if an
 *     experiment is active,
 *   - emits CSV telemetry at the configured rate.
 *
 * No motion ever leaves this task without going through
 * `can_task_set_tune_override()`, which is gated by RC failsafe +
 * VESC health watchdog + a 150 ms refresh timeout in can_task.c.
 */
...
/* Tune-CLI ERPM ceiling. Deliberately decoupled from MAX_WHEEL_ERPM
 * (which is the autonomous-mode safety cap) so we can sweep across
 * the motor's useful operating range during tuning without raising
 * the autopilot's speed limit. Stays well below the mcconf
 * l_max_erpm so the VESC's own limiter is never the gate. */
...
static void cmd_help(void)
{
    ...
    out_line("R,  quiet <on|off>   mute can_task ESP_LOG spam");
...
static void cmd_quiet(char *args)
{
    ...
    if (!strcmp(v, "on")) {
        esp_log_level_set("can_task", ESP_LOG_WARN);
        out_ok();
    } else if (!strcmp(v, "off")) {
        esp_log_level_set("can_task", ESP_LOG_INFO);
        out_ok();
    } else {
...
static void cmd_status(void)
{
    ...
    can_task_get_vesc_health(VESC_ID_LEFT,  &hL);
    can_task_get_vesc_health(VESC_ID_RIGHT, &hR);
    bool vesc_ok = hL.online && hR.online;

    vesc_status_t sL = {0}, sR = {0};
    can_task_get_vesc_status(VESC_ID_LEFT,  &sL);
    can_task_get_vesc_status(VESC_ID_RIGHT, &sR);
...
static void cmd_enable(void)
{
    ...
    can_task_set_tune_override(0, 0);
    out_ok();
}
...
static void cmd_chirp(char *args)
{
    ...
    if (!(amp > 0.0f) || amp > (float)ERPM_LIMIT) {
        out_err("amp out of range (0, MAX_WHEEL_ERPM]"); return;
    }
...
static void tune_cli_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (s_enabled) {
            ...
            can_task_set_tune_override(tgtL, tgtR);
        }
